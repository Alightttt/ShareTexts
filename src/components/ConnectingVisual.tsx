...
export function ConnectingVisual({ className }: { className?: string }) {
  return (
    <div className={cn('relative w-56 h-28', className)} aria-hidden>
      ...
      <div className="absolute left-6 right-6 top-1/2 h-px overflow-visible" style={{ ['--travel' as string]: '176px' }}>
        <span className="absolute left-0 top-1/2 w-2 h-2 rounded-full bg-azure-500 shadow-[0_0_12px_rgba(46,139,255,0.9)] animate-beam" />
        <span className="absolute left-0 top-1/2 w-1.5 h-1.5 rounded-full bg-azure-400 shadow-[0_0_10px_rgba(46,139,255,0.7)] animate-beam-delayed" />
      </div>
