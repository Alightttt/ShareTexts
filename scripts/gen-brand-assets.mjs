// One-shot deterministic brand asset generator. Renders the ember-brand OG
...
const EMBER_DK = '#fb9243'; // dark-mode ember

// The ShareText glyph: two rounded tiles joined by a diagonal beam, filled
// with the ember gradient (same geometry as ShareTextLogo.tsx).
function glyphSVG(size, scale = 1, flat = null) {
  const s = scale;
  const o = (256 - 256 * s) / 2;
  const fill = flat || 'url(#emberGrad)';
  return `<svg width="${size}" height="${size}" viewBox="0 0 256 256" fill="none" xmlns="http://www.w3.org/2000/svg">
  <defs><linearGradient id="emberGrad" x1="24" y1="20" x2="216" y2="236" gradientUnits="userSpaceOnUse">
    <stop offset="0" stop-color="${HONEY}"/><stop offset="0.55" stop-color="#f97b3d"/><stop offset="1" stop-color="${EMBER}"/>
  </linearGradient></defs>
  <rect x="${40 * s + o}" y="${36 * s + o}" width="${84 * s}" height="${84 * s}" rx="${24 * s}" fill="${fill}"/>
  <rect x="${132 * s + o}" y="${136 * s + o}" width="${84 * s}" height="${84 * s}" rx="${24 * s}" fill="${fill}"/>
  <path d="M${110 * s + o} ${106 * s + o} L${146 * s + o} ${142 * s + o}" stroke="${fill}" stroke-width="28" stroke-linecap="round"/>
</svg>`;
}
...
const ogHtml = `<!doctype html><html><head><meta charset="utf-8"><style>
  * { margin: 0; box-sizing: border-box; }
  ...
  .dots { position: absolute; inset: 0;
    background-image: radial-gradient(rgba(33,31,26,0.05) 1px, transparent 1px);
    background-size: 26px 26px; }
  .glow { position: absolute; top: -160px; left: -120px; width: 520px; height: 520px; border-radius: 9999px;
    background: rgba(254,171,48,0.16); filter: blur(110px); }
  ...
  h1 { font-size: 68px; font-weight: 800; line-height: 1.04; letter-spacing: -0.035em; color: ${INK}; }
  ...
  .url { margin-top: 34px; display: inline-flex; align-items: center; gap: 10px; padding: 12px 22px; border-radius: 999px;
    background: ${INK}; color: #fff; font-size: 20px; font-weight: 700; letter-spacing: -0.01em; }
  .art { position: absolute; right: 96px; top: 50%; transform: translateY(-50%); width: 300px; opacity: 0.95; }
</style></head><body>
  <div class="dots"></div><div class="glow"></div>
  <div class="content">
    <div class="brand">${glyphSVG(52)}<span class="brand-name">ShareText</span></div>
  ...
  <div class="art">
    <svg viewBox="0 0 220 220" fill="none" xmlns="http://www.w3.org/2000/svg">
      <defs><linearGradient id="emberGrad2" x1="18" y1="18" x2="202" y2="202" gradientUnits="userSpaceOnUse">
        <stop offset="0" stop-color="${HONEY}"/><stop offset="1" stop-color="${EMBER}"/>
      </linearGradient></defs>
      <rect x="18" y="18" width="82" height="82" rx="24" fill="${EMBER}" opacity="0.12" stroke="url(#emberGrad2)" stroke-width="6"/>
      <rect x="120" y="120" width="82" height="82" rx="24" fill="${EMBER}" opacity="0.12" stroke="url(#emberGrad2)" stroke-width="6"/>
      <path d="M86 84 L132 130" stroke="url(#emberGrad2)" stroke-width="14" stroke-linecap="round"/>
      <path d="M92 92 L112 84 L104 104 Z" fill="${EMBER}"/>
      <path d="M126 126 L104 136 L114 116 Z" fill="${EMBER}"/>
    </svg>
  </div>
</body></html>`;
...
function glyphAt(size, color, scale) {
  return glyphSVG(size, scale, color);
}
...
async function main() {
  ...
  await write(path.join(pub, 'icon-512.png'), pageHtml(512, CANVAS, glyphAt(512, null, 0.72), 0), 512, 512);
  await write(path.join(pub, 'icon-192.png'), pageHtml(192, CANVAS, glyphAt(192, null, 0.72), 0), 192, 192);
  await write(path.join(pub, 'icon-maskable-512.png'), pageHtml(512, EMBER, glyphAt(512, null, 0.55), 0), 512, 512);
  await write(path.join(pub, 'icon-maskable-192.png'), pageHtml(192, EMBER, glyphAt(192, null, 0.55), 0), 192, 192);
  await write(path.join(pub, 'apple-touch-icon.png'), pageHtml(180, CANVAS, glyphAt(180, null, 0.72), 0), 180, 180);

  // --- Favicons: transparent background, ember glyph (reads on any tab theme) ---
  ...
  </style></head><body>${glyphAt(size, null, 0.78)}</body></html>`;
