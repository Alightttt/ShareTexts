...
try {
  ...
  const landing = await A.evaluate(() => ({
    start: [...document.querySelectorAll('button')].some(b => b.textContent?.includes('Send')),
    haveCode: [...document.querySelectorAll('button')].some(b => b.textContent?.includes('Receive')),
  }));
